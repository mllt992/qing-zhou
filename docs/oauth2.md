...
- “登录请求已过期或失效”：状态有效期 10 分钟且只能使用一次，请从登录入口重新发起，不要刷新或重用认证中心回调链接。每次流程独立保存浏览器校验 Cookie，同一浏览器的多个标签页可以分别完成登录或绑定，不会相互覆盖。发起登录的页面须与配置的回调地址使用相同的 HTTPS 主机名；HTTP、不同域名或浏览器禁用 Cookie 都可能导致回调无法取得校验信息，即使认证中心已完成授权也无法建立轻舟会话。
